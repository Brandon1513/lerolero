                ...
                <div class="min-w-[160px]">
                    ...
                    <select name="origen_id"
                        ...
                        @foreach($almacenes as $almacen)
                            <option value="{{ $almacen->id }}" {{ request('origen_id') == $almacen->id ? 'selected' : '' }}>{{ $almacen->nombre }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="min-w-[160px]">
                    ...
                    <select name="destino_id"
                        ...
                        <option value="">Todos</option>
                        @foreach($almacenes as $almacen)
                            <option value="{{ $almacen->id }}" {{ request('destino_id') == $almacen->id ? 'selected' : '' }}>{{ $almacen->nombre }}</option>
                        @endforeach
                    </select>
                </div>
        ...
        <div class="grid grid-cols-2 gap-3 mb-4 sm:grid-cols-4">
            <div class="flex items-center gap-3 px-4 py-3 bg-white border border-gray-200 shadow-sm rounded-xl">
                <div class="flex items-center justify-center bg-indigo-100 rounded-lg w-9 h-9 shrink-0">
                    <svg class="w-5 h-5 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/></svg>
                </div>
                <div><div class="text-xl font-bold text-gray-900">{{ $statsTotal }}</div><div class="text-xs text-gray-500">Total traslados</div></div>
            </div>
            <div class="flex items-center gap-3 px-4 py-3 bg-white border border-gray-200 shadow-sm rounded-xl">
                ...
                <div><div class="text-xl font-bold text-gray-900">{{ $statsHoy }}</div><div class="text-xs text-gray-500">Hoy</div></div>
            </div>
            <div class="flex items-center gap-3 px-4 py-3 bg-white border border-gray-200 shadow-sm rounded-xl">
                ...
                <div><div class="text-xl font-bold text-gray-900">{{ $statsSemana }}</div><div class="text-xs text-gray-500">Esta semana</div></div>
            </div>
            <div class="flex items-center gap-3 px-4 py-3 bg-white border border-gray-200 shadow-sm rounded-xl">
                <div class="flex items-center justify-center bg-purple-100 rounded-lg w-9 h-9 shrink-0">
                    <svg class="w-5 h-5 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4"/></svg>
                </div>
                <div><div class="text-xl font-bold text-gray-900">{{ $statsProductosMovidos }}</div><div class="text-xs text-gray-500">Productos movidos hoy</div></div>
            </div>
        </div>
                        ...
                        @forelse($traslados as $traslado)
                            <tr class="transition-colors hover:bg-gray-50">
                                <td class="px-5 py-3.5">
                                    <div class="font-semibold text-gray-900">{{ \Carbon\Carbon::parse($traslado->fecha)->format('d/m/Y') }}</div>
                                    <div class="text-xs text-gray-400">{{ \Carbon\Carbon::parse($traslado->fecha)->diffForHumans() }}</div>
                                </td>
                                <td class="px-5 py-3.5">
                                    ...
                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold rounded-full
                                        {{ $tipoOrigen === 'vendedor' ? 'bg-blue-100 text-blue-800' : ($tipoOrigen === 'rechazo' ? 'bg-red-100 text-red-700' : ($tipoOrigen === 'general' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-700')) }}">
                                ...
                                <td class="px-5 py-3.5">
                                    ...
                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold rounded-full
                                        {{ $tipoDestino === 'vendedor' ? 'bg-blue-100 text-blue-800' : ($tipoDestino === 'rechazo' ? 'bg-red-100 text-red-700' : ($tipoDestino === 'general' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-700')) }}">
                                        {{ $traslado->destino->nombre }}
                                    </span>
                                </td>
                                <td class="px-5 py-3.5 text-gray-500 text-sm max-w-xs truncate">{{ $traslado->observaciones ?? '—' }}</td>
                                <td class="px-5 py-3.5">
                                    <div class="flex items-center justify-center gap-1.5">
                                        {{-- Ver detalle: siempre visible --}}
                                        <a href="{{ route('traslados.show', $traslado) }}" title="Ver detalle"
                                        ...
                                        {{-- Eliminar: solo si tiene permiso Y puede eliminarse --}}
                                        @can('traslados.eliminar')
                            ...
                            <tr>
                                <td colspan="5" class="px-5 py-16 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        <svg class="w-12 h-12 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/></svg>
