    ...
    <div class="py-8 mx-auto max-w-7xl sm:px-6 lg:px-8">
            ...
            <form method="GET" action="{{ route('promociones.index') }}" class="flex flex-wrap items-end gap-3">
                ...
                <div class="flex gap-2">
                    ...
                    @if(request()->hasAny(['buscar','estado','vigencia']))
                        <a href="{{ route('promociones.index') }}" class="inline-flex items-center gap-1.5 px-4 py-2 bg-gray-100 text-gray-700 text-sm font-semibold rounded-lg hover:bg-gray-200 transition-colors">
        ...
        <div class="grid grid-cols-2 gap-3 mb-4 sm:grid-cols-4">
            ...
            <div class="flex items-center gap-3 px-4 py-3 bg-white border border-gray-200 shadow-sm rounded-xl">
                ...
                <div><div class="text-xl font-bold text-gray-900">{{ $statsVigentes }}</div><div class="text-xs text-gray-500">Vigentes</div></div>
            </div>
            <div class="flex items-center gap-3 px-4 py-3 bg-white border border-gray-200 shadow-sm rounded-xl">
                ...
                <div><div class="text-xl font-bold text-gray-900">{{ $statsProximas }}</div><div class="text-xs text-gray-500">Próximas</div></div>
            </div>
            <div class="flex items-center gap-3 px-4 py-3 bg-white border border-gray-200 shadow-sm rounded-xl">
                ...
                <div><div class="text-xl font-bold text-gray-900">{{ $statsExpiradas }}</div><div class="text-xs text-gray-500">Expiradas</div></div>
            </div>
        </div>
        ...
        <div class="overflow-hidden bg-white border border-gray-200 shadow-sm rounded-xl">
            ...
            <div class="flex items-center justify-between px-5 py-3 border-b border-gray-100 bg-gray-50">
                <span class="text-sm text-gray-600">
                    Mostrando <span class="font-semibold text-gray-900">{{ $promociones->firstItem() ?? 0 }}</span>
                    – <span class="font-semibold text-gray-900">{{ $promociones->lastItem() ?? 0 }}</span>
                    de <span class="font-semibold text-gray-900">{{ $promociones->total() }}</span> promociones
                </span>
                <span class="text-xs text-gray-400">Página {{ $promociones->currentPage() }} de {{ $promociones->lastPage() }}</span>
            </div>
                        ...
                        @forelse($promociones as $promo)
                            @php
                                $tieneVentas = \Illuminate\Support\Facades\DB::table('venta_promociones')
                                    ->where('promocion_id', $promo->id)->exists();
                            ...
                            <tr class="hover:bg-gray-50 transition-colors {{ !$promo->activo ? 'opacity-60' : '' }}">
                                ...
                                <td class="px-5 py-3.5">
                                    <div class="flex items-start gap-3">
                                        <div class="flex items-center justify-center text-sm font-bold text-indigo-700 bg-indigo-100 rounded-lg w-9 h-9 shrink-0">
                                            {{ strtoupper(substr($promo->nombre, 0, 1)) }}
                                        </div>
                                        <div>
                                            <div class="font-semibold text-gray-900">{{ $promo->nombre }}</div>
                                            @if($promo->descripcion)
                                                <div class="text-xs text-gray-500 mt-0.5 max-w-[200px] truncate">{{ $promo->descripcion }}</div>
                                            @endif
                                            @if($tieneVentas)
                                                <span class="inline-flex items-center mt-1 px-2 py-0.5 text-xs font-semibold rounded-full bg-amber-100 text-amber-700">
                                                    Usada en ventas
                                                </span>
                                            @endif
                                        </div>
                                ...
                                <td class="px-5 py-3.5 text-right">
                                    <span class="font-bold text-gray-900">${{ number_format($promo->precio, 2) }}</span>
                                </td>
                                    ...
                                    <div class="flex flex-col gap-1">
                                        @forelse($promo->productos as $producto)
                                            <div class="flex items-center gap-1.5">
                                                <span class="flex items-center justify-center w-5 h-5 text-xs font-bold text-gray-600 bg-gray-100 rounded shrink-0">
                                ...
                                <td class="px-5 py-3.5">
                                    ...
                                    <span class="inline-flex items-center gap-1 px-2 py-0.5 text-xs font-semibold rounded-full {{ $vigColors[$vig] ?? 'bg-gray-100 text-gray-600' }}">
                                        <span class="w-1.5 h-1.5 rounded-full
                                            {{ $vig === 'vigente' ? 'bg-green-500' : ($vig === 'proxima' ? 'bg-blue-500' : 'bg-red-400') }}">
                                        </span>
                                ...
                                <td class="px-5 py-3.5">
                                    <div class="flex items-center justify-center gap-1.5">
                                        ...
                                        {{-- Editar --}}
                                        <a href="{{ route('promociones.edit', $promo) }}" title="Editar"
                                        ...
                                        <form action="{{ route('promociones.toggle', $promo) }}" method="POST" class="inline">
                                            @csrf @method('PATCH')
                                            <button type="submit" title="{{ $promo->activo ? 'Inactivar' : 'Activar' }}"
                                                class="inline-flex items-center justify-center w-8 h-8 rounded-lg transition-colors border
                                                    {{ $promo->activo ? 'bg-yellow-50 text-yellow-600 hover:bg-yellow-100 border-yellow-200' : 'bg-green-50 text-green-600 hover:bg-green-100 border-green-200' }}">
                                                @if($promo->activo)
                                        ...
                                        {{-- Eliminar --}}
                                        @if(!$tieneVentas)
                                            <form action="{{ route('promociones.destroy', $promo) }}" method="POST"
                                                onsubmit="return confirm('¿Eliminar la promoción {{ addslashes($promo->nombre) }}?')">
                                                @csrf @method('DELETE')
                                        ...
                                        @else
                                            <button type="button" title="No se puede eliminar: ya fue usada en ventas"
                            ...
                            <tr>
                                <td colspan="6" class="px-5 py-16 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <div class="font-medium text-gray-500">Sin promociones</div>
                                        <div class="text-sm text-gray-400">No hay promociones con los filtros aplicados.</div>
                                        @if(request()->hasAny(['buscar','estado','vigencia']))
                                            <a href="{{ route('promociones.index') }}" class="text-sm text-indigo-600 hover:underline">Limpiar filtros</a>
                                        @endif
                                    </div>
                                </td>
                            </tr>
            ...
            @if($promociones->hasPages())
                <div class="flex items-center justify-between px-5 py-4 border-t border-gray-100 bg-gray-50">
                    <div class="text-sm text-gray-600">{{ $promociones->total() }} resultados</div>
                    <div class="flex items-center gap-1">
                        ...
                        @else
                            <a href="{{ $promociones->previousPageUrl() }}&{{ http_build_query(request()->except('page')) }}" class="inline-flex items-center justify-center w-8 h-8 text-gray-600 transition-all border border-transparent rounded-lg hover:bg-white hover:shadow-sm hover:border-gray-200"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg></a>
                        @endif
                        @foreach($promociones->getUrlRange(max(1,$promociones->currentPage()-2), min($promociones->lastPage(),$promociones->currentPage()+2)) as $page => $url)
                            @if($page == $promociones->currentPage())
