<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-2xl font-bold tracking-tight text-gray-900">Editar Almacén</h2>
                <p class="text-sm text-gray-500 mt-0.5">{{ $almacen->nombre }}</p>
            </div>
        </div>
    </x-slot>
        ...
        <form method="POST" action="{{ route('almacenes.update', $almacen) }}">
            ...
            <div class="space-y-5">
                ...
                <div class="overflow-hidden bg-white border border-gray-200 shadow-sm rounded-xl">
                    ...
                    <div class="p-5 space-y-4">
                        @php $tipoActual = old('tipo', $almacen->tipo ?? 'general'); @endphp
                        <div>
                        ...
                        <div id="vendedor-field" class="{{ $tipoActual !== 'vendedor' ? 'hidden' : '' }}">
                            ...
                            <select name="user_id" id="user_id"
                                ...
                                @foreach($usuarios as $u)
                                    <option value="{{ $u->id }}" {{ old('user_id', $almacen->user_id) == $u->id ? 'selected' : '' }}>{{ $u->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
