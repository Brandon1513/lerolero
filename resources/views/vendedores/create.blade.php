                    ...
                    <div class="flex flex-wrap gap-3">
                        ...
                        @foreach($rolesDisponibles as $role)
                            <label class="flex items-center gap-3 px-4 py-3 border rounded-xl cursor-pointer transition-all
                                {{ in_array($role->name, $rolesActuales) ? 'border-indigo-300 bg-indigo-50' : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50' }}">
                                <input type="checkbox" name="roles[]" value="{{ $role->name }}"
                                    {{ in_array($role->name, $rolesActuales) ? 'checked' : '' }}
                                    {{ $role->name === 'administrador' ? 'onchange=handleAdminCheckbox(this)' : '' }}
                                    class="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500">
                                <div>
                                    <div class="text-sm font-semibold text-gray-800">{{ ucfirst($role->name) }}</div>
                                    <div class="text-xs text-gray-500">
                                        {{ $role->name === 'administrador' ? 'Acceso total al sistema' : 'Gestión de ventas y rutas' }}
                                    </div>
                                </div>
                            </label>
                        @endforeach
...
function handleAdminCheckbox(input) {
    if (input.checked) {
        const ok = confirm('⚠️ Estás a punto de asignar el rol de ADMINISTRADOR. ¿Estás seguro?');
