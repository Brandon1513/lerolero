<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-2xl font-bold tracking-tight text-gray-900">Editar Promoción</h2>
                <p class="text-sm text-gray-500 mt-0.5">{{ $promocion->nombre }}</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-4xl py-8 mx-auto sm:px-6 lg:px-8">
        ...
        @if($errors->any())
            <div class="flex items-start gap-3 px-4 py-3 mb-6 text-sm text-red-800 border border-red-200 bg-red-50 rounded-xl">
                ...
                <div><div class="mb-1 font-semibold">Errores:</div><ul class="list-disc list-inside space-y-0.5">@foreach($errors->all() as $e)<li>{{ $e }}</li>@endforeach</ul></div>
            </div>
        @endif
            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
                ...
                <div class="space-y-5 lg:col-span-2">
                        ...
                        <div class="p-5 space-y-4">
                            ...
                            <div>
                                ...
                                <textarea id="descripcion" name="descripcion" rows="2"
                                    class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none transition resize-none">{{ old('descripcion', $promocion->descripcion) }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="overflow-hidden bg-white border border-gray-200 shadow-sm rounded-xl">
                        <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gray-50">
                            ...
                            <span id="contador-productos" class="text-xs text-gray-400"></span>
                        </div>
                        <div class="p-5">
                            ...
                            <table class="w-full text-sm" id="tabla-productos">
                                ...
                                <tbody id="tbody-productos" class="divide-y divide-gray-100">
                                    @foreach($productosSeleccionados as $id => $cantidad)
                                        ...
                                        @if($producto)
                                            <tr id="producto_{{ $id }}" class="transition-colors hover:bg-gray-50">
                                                <td class="px-3 py-2.5">
                                                    <input type="hidden" name="productos[{{ $id }}][id]" value="{{ $id }}">
                                                    <div class="flex items-center gap-2">
                                                        <span class="flex items-center justify-center w-6 h-6 text-xs font-bold text-indigo-700 bg-indigo-100 rounded shrink-0">{{ strtoupper(substr($producto->nombre, 0, 1)) }}</span>
                                                        <span class="text-sm text-gray-800">{{ $producto->nombre }}</span>
                                                    </div>
                                                </td>
                                                <td class="px-3 py-2.5 text-center">
                                                    <input type="number" name="productos[{{ $id }}][cantidad]" value="{{ $cantidad }}" min="1" required
                            ...
                            <div id="empty-productos" class="hidden py-8 text-sm text-center text-gray-400">No hay productos agregados.</div>
                        </div>
                    </div>
                ...
                <div class="space-y-5">
                    ...
                    <div class="overflow-hidden bg-white border border-gray-200 shadow-sm rounded-xl">
                        ...
                        <div class="p-5">
                            <label for="precio" class="block text-sm font-medium text-gray-700 mb-1.5">Precio Promocional <span class="text-red-500">*</span></label>
                            <div class="relative">
                            ...
                            @error('precio')<p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>@enderror
                        </div>
                    </div>
    ...
    function actualizarContador() {
        ...
        if (filas === 0) { empty.classList.remove('hidden'); thead.style.display = 'none'; }
        else { empty.classList.add('hidden'); thead.style.display = ''; }
    }
