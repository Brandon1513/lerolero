    ...
    <div class="max-w-2xl py-8 mx-auto sm:px-6 lg:px-8">
        ...
        <form method="POST" action="{{ route('clientes.update', $cliente) }}" class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="asignado_a" class="w-full px-3 py-2 text-sm transition bg-white border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-indigo-500">
                                ...
                                @foreach($vendedores as $v)
                                    <option value="{{ $v->id }}" {{ old('asignado_a', $cliente->asignado_a) == $v->id ? 'selected' : '' }}>{{ $v->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="nivel_precio_id" class="w-full px-3 py-2 text-sm transition bg-white border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-indigo-500">
                                ...
                                @foreach($niveles as $nivel)
                                    <option value="{{ $nivel->id }}" {{ old('nivel_precio_id', $cliente->nivel_precio_id) == $nivel->id ? 'selected' : '' }}>{{ $nivel->nombre }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="p-5 space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" id="latitud" name="latitud" value="{{ old('latitud', $cliente->latitud) }}"
                        ...
                        <div>
                            ...
                            <input type="text" id="longitud" name="longitud" value="{{ old('longitud', $cliente->longitud) }}"
            ...
            <div class="overflow-hidden bg-white border border-gray-200 shadow-sm rounded-xl">
                ...
                <div class="p-5">
                    @php
                        $diasSel = old('dias_visita', $cliente->dias_visita ?? []);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    btn.addEventListener('click', () => {
        ...
        btn.disabled = true;
        btn.innerHTML = `<svg class="w-3.5 h-3.5 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg> Obteniendo...`;
        navigator.geolocation.getCurrentPosition(
            (pos) => {
                ...
                btn.disabled = false;
                btn.innerHTML = `✅ Ubicación obtenida (±${Math.round(pos.coords.accuracy)}m)`;
                setTimeout(() => { btn.innerHTML = `<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg> Usar mi ubicación actual`; }, 4000);
            },
            (err) => {
                btn.disabled = false;
